...
def test_idn():
	idn = inst.query("*idn?")
	#idn = idn.replace("\n", "n")
	#print(idn + "len" + str(len(idn)))
	assert (idn == "LTE,probeInterface,G2,dev\r\n")
...
def test_trig():
	...
	stb = inst.read_stb()
	#print(stb)
	assert (stb & 0x40), "SRQ not set after 0.3 seconds"
	assert (inst.read_stb() == 0)
...
def test_mav():
	...
	rsp = inst.read()
	assert(rsp == "123\r\n")
	assert(inst.read_stb() == 0), "MAV not back to 0"
...
def test_srq():
	...
	#inst.wait_for_srq()
        ...
        stb = inst.read_stb()
        #print(stb)
        assert (stb == 0x10), "SRQ set at second read!"
        ...
        stb = inst.read_stb()
        #print(stb)
        assert (stb == 0x00), "STB bits still set after read"
...
def test_multi_read():
	...
	msg = "0123456789abcdefghijklmnopqrstuvwxyz" * 13
	longstr = "ECHO \"" + msg + "\""
	timeout = 10
	x = longstr
	#x = longstr[0:174]
	inst.chunk_size = 40 # Seems chunk size only applies to read but not write
	inst.write(x)
	time.sleep(1)
...
inst.write("*CLS") #clear errors from last interation
inst.clear()
#print(inst.query("SYST:ERR:COUN?"))
...
#we do not have delay functions and cannot do this test
#print("+test abort in")
#test_abort_in()
